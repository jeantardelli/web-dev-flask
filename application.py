...
dotenv_path = os.path.join(os.path.dirname(__file__), '.env-server')
...
def make_shell_context():
    return dict(db=db, Subscriber=Subscriber, Letter=Letter)
...
def test(coverage, test_names):
    ...
    if test_names:
        tests = unittest.unittest.TestLoader().loadTestsFromNames(test_names)
    else:
